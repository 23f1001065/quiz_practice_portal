            ...
            <div class="container-fluid h-100" v-bind:class="{'blurr-back': is_clicked_edit_icon}">
                <div class="row h-100">
                            ...
                            <table class="table table-striped align-middle overflow-auto">
                                ...
                                <tbody>    
                                    <tr v-for="(subject,index) in subjects" class="p-1  subject">
                                        ...
                                        <td class="text-wrap"><i class="bi bi-pencil-square text-warning" @click="setId(subject.id, subject.name, subject.description)"></i></td>
                    ...
                    <div class="col-sm-3 border h-100 p-2 pt-3">
                        <button v-if="!is_clicked" class="button" v-on:click="add_subject"><i class="bi bi-plus-circle fs-2"></i>Add New Subject</button>
                        <div v-if="is_clicked" class="mt-4">
                            ...
                            <form v-on:submit.prevent="createSubject">
                                ...
                                <button type="submit" class="button form-control">Create</button><br>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <div v-if="is_clicked_edit_icon" class="m-5 w-25 z-1 top-0 end-0 position-absolute blurr-contain border">
                ...
                <form v-on:submit.prevent="editSub(id_to_edit)">
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <span class="fs-5" @click="cancel"><i class="bi bi-x-lg"></i></span>
                    </div>
                </form>
            </div>
        </div>
    ...
    methods: {
        ...
        async deleteSub(id) {
            ...
            if (userResponse) {
                ...
                const data = await response.json()
                if (response.ok) {
                    let index = this.subjects.findIndex(sub => sub.id === id)
